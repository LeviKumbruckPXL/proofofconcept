...
export default defineConfig({
    plugins      : [react()],
    optimizeDeps : {
        include : ['@bryntum/calendar', '@bryntum/calendar-angular']
    }
});
```

## A property added to calendarProps has no effect
...
Angular does not process `calendarProps` file as a whole, but we need to use individual properties in the template.
