...
const calendar = new Calendar({
    ...
    performanceRenderer({
        ...
        return [{
            ...
            text : `${isSmallDevice ? '' : 'From'} ${costFormatter.format(eventRecord.cost || ((_eventRecord$resource = eventRecord.resource) === null || _eventRecord$resource === undefined ? undefined : _eventRecord$resource.minCost) || 0)}`
        }];
    },
    // If the show selection combo is cleared, display all in the input field
    showValueRenderer(show) {
        return (show === null || show === undefined ? undefined : show.name) || 'All';
    },
    onShowSelect({
        value
    }) {
...
calendar.widgetMap.showSelector.store = calendar.resourceStore.chain();
