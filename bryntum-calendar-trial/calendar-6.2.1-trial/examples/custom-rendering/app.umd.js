...
class MultiDays extends DayView {
    ...
    get description() {
        ...
        if (DateHelper.diff(startDate, endDate, 'd') > 1) {
            const startMonth = DateHelper.getMonthName(startDate.getMonth()),
    ...
    onShow() {
        ...
        const {
            datePicker
        } = this.calendar.widgetMap;
        (_super$onShow = super.onShow) === null || _super$onShow === undefined || _super$onShow.call(this, ...arguments);

        // Set the Calendar date picker's range to be the range we cover
        datePicker.setConfig({
            multiSelect : 'range',
            selection   : [this.startDate, DateHelper.add(this.endDate, -1)]
        });
    ...
    onHide() {
        var _this$dateListener, _super$onHide;
        this.calendar.widgetMap.datePicker.multiSelect = false;
        (_this$dateListener = this.dateListener) === null || _this$dateListener === undefined || _this$dateListener.call(this);
        (_super$onHide = super.onHide) === null || _super$onHide === undefined || _super$onHide.call(this);
    }
