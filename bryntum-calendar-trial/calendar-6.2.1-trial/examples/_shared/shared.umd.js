...
const bryntumUrl = 'https://bryntum.com',
  ...
  noEditor = window.matchMedia('(max-width: 700px)').matches;
if (!isDemoBrowser) {
      ...
      codepenButton = new Button({
        ...
        async onClick({
          ...
          const {
            ...
            productId = window.bryntum.product.name,
            productUrl = `${bryntumUrl}/products/${productId}`;
          let {
            codeEditor
          } = me;
          if (!codeEditor) {
          ...
          function refineHTMLSnippet(html) {
            ...
            [...codePenHTML.querySelectorAll('script')].forEach(script => {
              ...
              if (src && (src.includes('app.module.js') || src.includes('../_shared/locales/examples.locales.umd.js'))) {
                script.remove();
              }
            });
          ...
          const loadedCode = ((_codeEditor = codeEditor) === null || _codeEditor === undefined ? undefined : _codeEditor.codeCache['app.js']) || ((_codeEditor2 = codeEditor) === null || _codeEditor2 === undefined ? undefined : _codeEditor2.codeCache['app.module.js']),
            ...
            updatedCode = loadedCode.replace(/^\s*import\s+.*?;.*$/gm, '')
            ...
            themeLink = document.querySelector('link[data-bryntum-theme="true"]'),
            ...
            stylings = (((_me$appConfig$index = me.appConfig.index) === null || _me$appConfig$index === undefined ? undefined : _me$appConfig$index.stylesheets) ?? []).map(({
              href
            ...
            formConfig = {
              ...
              children: [{
                ...
                value: StringHelper.safeJsonStringify({
                  ...
                  html: ['<html lang="en">', '<head>', `<link data-bryntum-theme="true" rel="stylesheet" href="https://bryntum.com/products/${productId}/build/${productId}.${themeName}.css?${hash}">`, `<link rel="stylesheet" href="${productUrl}/examples/_shared/shared.css?${hash}">`, stylings, '</head>', '<body>', htmlBody, me.localeManager.locale.localeName !== 'En' ? `<script src="https://bryntum.com/products/${productId}/examples/_shared/locales/examples.locale.${me.localeManager.locale.localeName}.umd.js?${hash}"></script>` : '', '</body>', '</html>'].join('\n'),
                  js: [`document.body.classList.add("b-theme-${themeName}"); // for styling purposes`, `import * as Module from '${productUrl}/build/${productId}.module.js?${hash}';`, 'Object.assign(window, Module);', '// The code above imports module bundle and places all Bryntum classes on window to simplify coding at CodePen\n', updatedCode, me.localeManager.locale.localeName !== 'En' ? `LocaleManager.applyLocale('${me.localeManager.locale.localeName}'); // Apply localization` : ''].join('\n')
                })
              }]
            },
