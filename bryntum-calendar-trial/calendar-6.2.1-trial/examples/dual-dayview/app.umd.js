...
class DualDayView extends Panel.mixin(CalendarMixin, DaySelectable) {
    ...
    static #_ = _defineProperty(this, 'timeAxisConfigs', ['allDayEvents'].concat(this.sharedConfigs));
    construct(config) {
        config.items = [{
            ...config,
            ...config.primaryView,
    ...
    // Initialize Calendar Features to treat the source view correctly
        if (firstPaint) {
            ...
            (_features$eventToolti = features.eventTooltip) === null || _features$eventToolti === undefined || _features$eventToolti.tooltip.on({
                beforeAlign({
                    source
                }) {
            ...
            (_features$scheduleMen = features.scheduleMenu) === null || _features$scheduleMen === undefined || _features$scheduleMen.menu.on({
                beforeShow() {
    ...
    changeDayTime(dayTime) {
        ...
        if (!((_this$_dayTime = this._dayTime) !== null && _this$_dayTime !== undefined && _this$_dayTime.equals(dayTime))) {
            return dayTime;
        }
    }
    ...
    changeMonth(month) {
        ...
        if (!((_month = month) !== null && _month !== undefined && _month.isMonth)) {
            month = new Month({
                date,
   ...
   */
    ...
    onChildAdd(child) {
        ...
        // Do not count the DaySelector added by the DaySelectable mixin as a child view.
        if (!child.isDaySelector) {
    ...
    // is the result of a syncViewConfig call and will be applied to all views anyway.
        if (applied === 2 && !this.syncingChildViewConfigs && !this.syncingViewConfig && this.constructor.timeAxisConfigs.includes(name)) {
    ...
    scrollTo(target) {
        ...
        if (target.isEventModel) {
            ...
            return owningView === null || owningView === undefined ? undefined : owningView.scrollTo(target);
        }
...
class DualDayViewZone extends Base {
    ...
    changeZones(zones, oldZones) {
        if (oldZones !== null && oldZones !== undefined && oldZones.length && !zones) {
            for (let i = 0, {
                length
            } = oldZones; i < length; i++) {
                oldZones[i].destroy();
