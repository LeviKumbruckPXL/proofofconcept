...
AjaxHelper.mockUrl('dynamic-load', (url, urlParams, {
    queryParams
}) => {
    ...
    Toast.show(`Server request for ${DateHelper.format(startDate, 'Do MMMM YYYY')} to ${DateHelper.format(endDate, 'Do MMMM YYYY')}`);
    return {
        delay        : 100,
...
AjaxHelper.mockUrl('dynamic-load-sync', (url, urlParams, {
    body,
    queryParams
}) => {
    ...
    updated === null || updated === undefined || updated.forEach(e => {
    ...
    removed === null || removed === undefined || removed.forEach(e => {
    // load handler filters these out
        sourceRecords[e.id].removed = true;
    });
    added === null || added === undefined || added.forEach(e => {
