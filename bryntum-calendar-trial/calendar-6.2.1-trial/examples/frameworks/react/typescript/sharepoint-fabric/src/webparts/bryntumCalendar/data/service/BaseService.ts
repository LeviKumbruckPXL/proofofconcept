...
/**
 * This class interacts between Calendar's CrudManager and the SharePoint Task List by using a list proxy. It
...
export default class BaseService {
    ...
    /**
     * Returns the reference to the Calendar view
     * https://bryntum.com/products/calendar/docs/api/Calendar/view/Calendar
     */
    get calendar() {
    ...
    /**
     * Sets the listId (guid), loads the model
     * @param id
     */
    set listId(id) {
    ...
    private getTaskListUpdatePackage(): UpdatePackage {
        ...
        const events: any = this.model.eventStore.changes;

        // Process added, modified and removed records
        events?.added?.forEach(task => updatePackage.addActionToUpdatePackage(task, getChanges(task.toJSON())));

        events?.modified?.forEach(task => updatePackage.addActionToUpdatePackage(task, getChanges(task.modificationData)));

        events?.removed?.forEach(task => updatePackage.addActionToUpdatePackage(task, {}, true));
