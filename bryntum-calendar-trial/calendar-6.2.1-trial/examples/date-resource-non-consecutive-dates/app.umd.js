...
class EventSummaryPopup extends Popup {
    ...
    construct() {
        ...
        (_calendar$features$dr = calendar.features.drag) === null || _calendar$features$dr === undefined || _calendar$features$dr.on({
    ...
    async onRemoveEventsClick() {
        ...
        if ((await MessageDialog.confirm({
            title   : 'Delete events?',
            message : 'Do you you want to delete the events?'
    ...
    syncValues() {
        ...
        if (alignTo) {
            const values = [calendar.eventStore.count, selectedEvents.length, calendar.eventStore.reduce((v, event) => v + event.value || 0, 0), selectedEvents.reduce((v, event) => v + event.value || 0, 0)];
            DomSync.sync({
...
const calendar = new Calendar({
    ...
    // Existing sidebar widgets can be customized and extra UI Widgets can be easily added too
        items : {
            datePicker : {
                ...
                selection   : ['2025-04-01', '2025-04-02', '2025-04-04', '2025-04-07', '2025-04-08'],
                tbar        : {
    ...
    renderResourceAppointmentSummary(resource, {
        resourceDayEvents,
        key
    }) {
        const [dayValue, dayTravelTime] = resourceDayEvents.reduce((v, eventRecord) => {
...
calendar.eventSummaryPopup = new EventSummaryPopup({
    calendar
});
