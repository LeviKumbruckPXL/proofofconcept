...
const eventClasses = ['Calendar', 'DayView', 'MonthView', 'YearView', 'EventList', 'AgendaView', 'ResourceView', 'DayResourceView', 'EventSelection', 'Panel', 'Container', 'Widget', 'Grid', 'GridBase', 'GridElementEvents', 'RowManager'],
    docClasses = new Map(),
...
const fetchDocs = async docsJson => {
    const docsDataResponse = await fetch(docsJson),
    ...
    for (const cls of docsData.classes) {
        ...
        if (!docClasses.get(cls.name) || (_cls$modulePath = cls.modulePath) !== null && _cls$modulePath !== undefined && _cls$modulePath.startsWith('Calendar')) {
...
class EventLog extends Panel {
    ...
    onLogTargetClick({
        ...
        if (target.textContent.startsWith('#')) {
            ...
            const el = (_Rectangle$from = Rectangle.from(target)) === null || _Rectangle$from === undefined ? undefined : _Rectangle$from.visualize({
    ...
    // Try to filter down the event stream a little to make it useful
        if (!this.isPainted || eventName.match(blockedEventRe) || eventName === 'navigate' && !eventRecord || modifierKeys[domEvent === null || domEvent === undefined ? undefined : domEvent.key]) {
            return;
        }
        const calendar = this.up('calendar'),
            filter = this.widgetMap.eventNameFilter.value,
            target = !(month || week || date) && (t || (domEvent === null || domEvent === undefined ? undefined : domEvent.target)),
        ...
        if (item) {
            const newRecord = calendar.activeView.getEventRecord(item),
    ...
    log(eventName, message, target) {
        ...
        for (let i = 0, {
            length
        } = eventClasses; i < length && !docsUrl; i++) {
            var _docClasses$get;
            if ((_docClasses$get = docClasses.get(eventClasses[i])) !== null && _docClasses$get !== undefined && (_docClasses$get = _docClasses$get.eventsMap) !== null && _docClasses$get !== undefined && _docClasses$get.get(eventName.toLowerCase())) {
...
const calendar = new Calendar({
    ...
    onAddCalendarClick() {
        const {
            ...
            [newCalendar] = this.resourceStore.add({
                name : 'New calendar'
            }),
